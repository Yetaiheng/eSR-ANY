...
#每个block单独使用一个预测网络，预测网络由四层general conv 组成
...
class ECB_ada(nn.Module):
    def __init__(self, inp_planes, out_planes, depth_multiplier, act_type='prelu', with_idt = False):
        super(ECB_ada, self).__init__()
        ...
        backbone = []
        backbone += [BasicBlock_conv(8, 12)]
        backbone += [BasicBlock_conv(12, 24)]
        backbone += [BasicBlock_conv(24, 36)]
        backbone += [BasicBlock_conv(36, 36)]
        self.backbone = nn.Sequential(*backbone)
        self.getWeight = nn.Linear(36, 5,bias=True)
    ...
    def forward(self, x):
        ...
        y = self.conv3x3(x) * weight1.view(B,1,1,1)    + \
            self.conv1x1_3x3(x)* weight2.view(B,1,1,1) + \
            self.conv1x1_sbx(x)* weight3.view(B,1,1,1) + \
            self.conv1x1_sby(x)* weight4.view(B,1,1,1) + \
            self.conv1x1_lpl(x)* weight5.view(B,1,1,1)
        if self.with_idt:
            y += x
...
class ECBSR_5_ada(nn.Module):
    def __init__(self):
        super(ECBSR_5_ada, self).__init__()
        self.module_nums = 4
        self.channel_nums = 8
        self.scale = 2
        self.colors = 1
        ...
        for i in range(self.module_nums):
            backbone += [ECB_ada(self.channel_nums, self.channel_nums, depth_multiplier=2.0, act_type=self.act_type, with_idt = self.with_idt)]
