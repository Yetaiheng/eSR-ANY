...
import time
#局部的系数提取，每个block一个预测网络代码，两个，cat了输入图像的和没有cat的
class SeqConv3x3(nn.Module):#删去了self.scale
...
class ECBSR_5_ada_share_local(nn.Module):
    def __init__(self):
        super(ECBSR_5_ada_share_local, self).__init__()
        self.module_nums = 4
        self.channel_nums = 8
        self.scale = 2
        self.colors = 1
...
class ECB_ada_DW_cat(nn.Module):
    def __init__(self, inp_planes, out_planes, depth_multiplier, act_type='prelu', with_idt = False):
        super(ECB_ada_DW_cat, self).__init__()
        ...
        backbone = []
        #backbone += [BasicBlock_conv_no_down(1, 8)]
        backbone += [BasicBlock_conv(9, 20)]################### 9
        self.backbone = nn.Sequential(*backbone)
        self.getWeight = nn.Linear(20, 5,bias=True)
    ...
    def forward(self, x, shortcut):
        B,C,H,W = x.size()
        x1 = torch.cat((shortcut,x),dim = 1)
        weight = self.backbone(x1)
        weight = weight.mean([-2, -1])# N,C,H,W  ->  N,C
        weight = self.getWeight(weight)
        #print(weight.data)
        # #weight = self.softmax(weight)*2
...
class ECBSR_5_ada_share_local_cat(nn.Module):
    def __init__(self):
        super(ECBSR_5_ada_share_local_cat, self).__init__()
        self.module_nums = 4
        self.channel_nums = 8
        self.scale = 2
        self.colors = 1
        ...
        self.head= ECB(self.colors, self.channel_nums, depth_multiplier=2.0, act_type=self.act_type, with_idt = self.with_idt)
        for i in range(self.module_nums):
            backbone += [ECB_ada_DW_cat(self.channel_nums, self.channel_nums, depth_multiplier=2.0, act_type=self.act_type, with_idt = self.with_idt)]
        self.tail = ECB(self.channel_nums, self.colors*self.scale*self.scale, depth_multiplier=2.0, act_type='linear', with_idt = self.with_idt)
