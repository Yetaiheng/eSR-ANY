...
import time
#三种方法（ECBSR，全局系数提取，局部系数提取）的patch并行计算重参数化模型代码，只做测试使用
...
class ECBSR_5_ada_share_patch_re(nn.Module):
    def __init__(self):
        super(ECBSR_5_ada_share_patch_re, self).__init__()
        self.module_nums = 4
        self.channel_nums = 8
        self.scale = 2
        self.colors = 1
        ...
        # backbone_weight += [BasicBlock_conv_no_down(16, 20)]
        # backbone_weight += [BasicBlock_conv_no_down(8, 16)]
        # backbone_weight += [BasicBlock_conv_no_down(16, 32)]
        self.backbone_weight = nn.Sequential(*backbone_weight)
        self.getWeight1 = nn.Linear(20, 20,bias=True)
        #self.getWeight2 = nn.Linear(20, 20,bias=True)
    ...
    def forward(self, x):
        ...
        #weight = self.getWeight2(weight)
        #weight1, weight2,weight3,weight4 = torch.split(weight, [5,5,5,5], dim=1)
        shortcut = x
        x = self.head(x)
...
class ECBSR_5_ada_share_local_patch_re(nn.Module):
    def __init__(self):
        super(ECBSR_5_ada_share_local_patch_re, self).__init__()
        self.module_nums = 4
        self.channel_nums = 8
        self.scale = 2
        self.colors = 1
...
if __name__ == '__main__':
    z = torch.randn((3, 1, 64, 64)).cuda()
    
    model = ECBSR_5_ada_share_local_patch_re().cuda()
    model.eval()
    z = model(z)
    print(z.shape)
